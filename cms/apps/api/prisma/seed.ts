...
async function main() {
    ...
    const headerMenuItems = [
        ...
        { id: 'nav-5', label: 'Diagnostic', url: 'audit-ia.html', order: 4, location: 'HEADER' },
    ];
    ...
    const faqs = [
        {
            ...
            answer: "Le budget dépend du processus, des outils à connecter et du niveau de personnalisation. Nous commençons par un diagnostic pour prioriser les gains, cadrer la solution et estimer le budget avant tout engagement.",
            order: 0,
        },
        {
            id: 'faq-2',
            question: 'Combien de temps faut-il pour obtenir de premiers résultats ?',
            answer: "Une première automatisation ou intégration ciblée peut être livrée en quelques semaines. Les programmes plus larges sont déployés progressivement afin de produire des gains rapides sans perturber les opérations.",
            order: 1,
        },
        {
            id: 'faq-3',
            question: 'Avons-nous besoin de beaucoup de données pour commencer ?',
            answer: "Non. De nombreuses automatisations et intégrations démarrent sans historique important. Lorsqu’un cas d’usage IA exige des données, le diagnostic évalue leur qualité et propose une trajectoire réaliste.",
            order: 2,
        },
        {
            id: 'faq-4',
            question: "Faut-il nécessairement utiliser de l’IA ?",
            answer: "Non. Nous privilégions toujours la solution la plus simple et la plus fiable : automatisation classique, intégration API, amélioration de l’ERP ou IA lorsqu’elle apporte une valeur mesurable.",
            order: 3,
        },
        {
            id: 'faq-5',
            question: 'Travaillez-vous avec les PME ou seulement les grands groupes ?',
            answer: "Nous travaillons principalement avec les PME et ETI. Notre expertise consiste à adapter l’automatisation, l’intégration et l’IA à leurs outils, contraintes et budgets.",
            order: 4,
        },
        {
            id: 'faq-6',
            question: 'Comment limitez-vous le risque du projet ?',
            answer: "Nous commençons par un périmètre ciblé, des objectifs mesurables et une validation rapide. Le déploiement ne s’élargit qu’après confirmation des gains et de la fiabilité.",
            order: 5,
        },
    ];
    ...
    const settings = [
        ...
        { id: 'set-2', key: 'site_tagline', value: 'Automatisation, intégration & IA au service des opérations', type: 'string' },
    ...
    const pages = [
        {
            ...
            status: 'PUBLISHED',
            sections: [
                { type: 'hero', order: 0, content: JSON.stringify({ title: "Transformez vos processus métiers", subtitle: "Nous automatisons vos tâches, connectons vos outils et intégrons l’IA lorsqu’elle apporte une valeur mesurable.", badge: 'Automatisation · Intégration · IA', cta: { text: 'Identifier mes gains potentiels', url: 'audit-ia.html' } }) },
                { type: 'stats', order: 1, content: JSON.stringify([{ value: '15+', label: 'Projets opérationnels déployés' }, { value: '6', label: "Secteurs d'expertise" }, { value: '40%', label: 'Gain de temps moyen' }]) },
                ...
                { type: 'cta', order: 6, content: JSON.stringify({ title: "Quels gains se cachent dans vos processus ?", buttonText: 'Démarrer le diagnostic', buttonUrl: 'audit-ia.html' }) },
            ],
        },
        {
            id: 'page-solutions',
            ...
            status: 'PUBLISHED',
            sections: [
                { type: 'hero', order: 0, content: JSON.stringify({ title: "Automatisez, connectez et exploitez mieux vos données", subtitle: "Astauria transforme les processus des PME grâce à l’automatisation, l’intégration des systèmes et l’IA utile." }) },
                ...
                { type: 'solution', order: 2, content: JSON.stringify({ icon: 'refresh-cw', title: 'Automatisation des processus', problem: "Trop de tâches manuelles, répétitives et sources d’erreurs.", solution: 'Nous automatisons la facturation, les documents, les validations et les échanges entre outils.', results: ['Réduction du temps administratif', 'Moins d’erreurs humaines', 'Processus plus fiables'] }) },
                { type: 'solution', order: 3, content: JSON.stringify({ icon: 'plug', title: 'Intégration ERP, CRM & API', problem: 'Vos systèmes ne communiquent pas et imposent des doubles saisies.', solution: 'Nous connectons vos applications et synchronisons les données utiles au bon moment.', results: ['Information cohérente', 'Flux continus', 'Meilleure visibilité'] }) },
                { type: 'solution', order: 4, content: JSON.stringify({ icon: 'bot', title: 'IA & exploitation des données', problem: 'Vos documents et données restent difficiles à analyser ou à exploiter.', solution: 'Nous déployons des assistants, analyses et modèles ciblés lorsqu’ils apportent un avantage mesurable.', results: ['Décisions plus rapides', 'Analyse augmentée', 'IA maîtrisée'] }) },
                { type: 'methodology', order: 5, content: JSON.stringify({ title: 'Une méthodologie claire et maîtrisée', steps: [{ number: 1, title: 'Diagnostic opérationnel', desc: 'Analyse des processus, outils et opportunités' }, { number: 2, title: 'Solution ciblée', desc: 'Validation rapide sur un cas concret' }, { number: 3, title: 'Déploiement progressif', desc: 'Intégration dans vos outils existants' }, { number: 4, title: 'Accompagnement & support', desc: 'Suivi, ajustements et amélioration continue' }] }) },
                { type: 'cta', order: 6, content: JSON.stringify({ title: 'Une entrée sans risque', benefits: ["Vision claire avant d’investir", 'Recommandations exploitables', 'Diagnostic offert si projet lancé'], buttonText: 'Identifier mes gains potentiels', buttonUrl: 'audit-ia.html' }) },
            ],
        },
        {
            id: 'page-pourquoi',
            ...
            status: 'PUBLISHED',
            sections: [
                { type: 'hero', order: 0, content: JSON.stringify({ title: "Un partenaire de confiance pour transformer les opérations des PME", subtitle: "Astauria accompagne les PME en France et au Cameroun avec des solutions d’automatisation, d’intégration et d’IA pragmatiques, orientées résultats." }) },
                { type: 'conviction', order: 1, content: JSON.stringify({ title: "La technologie n’a de valeur que si elle produit des résultats", intro: "Automatisation, intégration ou IA doivent avant tout :", points: ['Simplifier le travail des équipes', 'Fiabiliser les processus', 'Aider à prendre de meilleures décisions'], highlight: 'Notre approche est concrète, mesurée et responsable.' }) },
                ...
                { type: 'cta', order: 7, content: JSON.stringify({ title: 'Une approche responsable et sans risque', buttonText: 'Démarrer le diagnostic', buttonUrl: 'audit-ia.html' }) },
            ],
        },
        {
            id: 'page-cas-usage',
            ...
            status: 'PUBLISHED',
            sections: [
                { type: 'hero', order: 0, content: JSON.stringify({ title: "La transformation en action : des résultats concrets", subtitle: "Découvrez comment nous automatisons les processus, connectons les systèmes et utilisons l’IA lorsqu’elle crée une valeur mesurable." }) },
        ...
        },
        {
            id: 'page-audit',
            slug: 'audit-ia',
            title: 'Diagnostic Automatisation, Data & IA',
            template: 'audit',
            status: 'PUBLISHED',
            sections: [
                { type: 'hero', order: 0, content: JSON.stringify({ title: "Diagnostic Automatisation, Data & IA", subtitle: "En 30 minutes, identifiez les opportunités adaptées à vos processus, vos outils et vos données. Sans engagement.", badge: 'Offert si projet lancé' }) },
                ...
                { type: 'process', order: 2, content: JSON.stringify({ title: "Comment ça marche ?", steps: ['Questionnaire ciblé (5 min)', 'Échange diagnostic (30 min)', 'Restitution personnalisée'] }) },
                { type: 'form', order: 3, content: JSON.stringify({ title: "Démarrer votre diagnostic", fields: ['Objectif', 'Priorités', 'Nom', 'Email', 'Entreprise', 'Téléphone'] }) },
            ],
        },
        {
            id: 'page-blog',
